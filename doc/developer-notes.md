...
- [Development guidelines](#development-guidelines)
    - [General Amero Core](#general-amero-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Amero Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/ameropay/amero/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_amero
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_amero --log_level=test_suite
$ valgrind -v --leak-check=full src/amerox -printtoconsole
```
...
# A coverage report will now be accessible at `./test_amero.coverage/index.html`.
```

**Sanitizers**

Amero Core can be compiled with various "sanitizers" enabled, which add
...
be sent upstream but bugfixes may also be prudent to PR against Amero Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `amero-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `amero-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Amero Core, the wallet was always
