...
bool AppInit(int argc, char* argv[])
{
    ...
    if (gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||  gArgs.IsArgSet("-help") || gArgs.IsArgSet("-version"))
    {
        ...
        {
            strUsage += "\nUsage:\n"
                  "  amerod [options]                     " + strprintf("Start %s Daemon", PACKAGE_NAME) + "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                fprintf(stderr, "Error: Command line contains unexpected token '%s', see amerod -h for a list of options.\n", argv[i]);
                return false;
            }
        }

        // -server defaults to true for amerod but not for the GUI so do this here
