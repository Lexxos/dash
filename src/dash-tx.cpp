...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf("%s amero-tx utility version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
            "Usage:\n"
              "  amero-tx [options] <hex-tx> [commands]  Update hex-encoded amero transaction\n" +
              "  amero-tx [options] -create [commands]   Create hex-encoded amero transaction\n" +
              "\n";
