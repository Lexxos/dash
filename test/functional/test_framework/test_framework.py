...
class BitcoinTestFramework():
    ...
    def main(self):
        ...
        parser.add_option("--srcdir", dest="srcdir", default=os.path.abspath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"),
                          help="Source directory containing amerod/dash-cli (default: %default)")
        ...
        parser.add_option("--amerod-arg", dest="amerod_extra_args", default=[], type='string', action='append',
                          help="Pass extra args to all amerod instances")
        ...
        self.options.bitcoind = os.getenv("BITCOIND", default=config["environment"]["BUILDDIR"] + '/src/amerod' + config["environment"]["EXEEXT"])
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=config["environment"]["BUILDDIR"] + '/src/dash-cli' + config["environment"]["EXEEXT"])

        self.extra_args_from_options = self.options.amerod_extra_args
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: amerods were not stopped and may still be running")
    ...
    def stop_node(self, i, wait=0):
        """Stop a amerod test node"""
        self.nodes[i].stop_node(wait=wait)
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self, wait=0):
        """Stop multiple amerod test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as amerod's debug.log with microprecision (so log files can be concatenated and sorted)
...
def skip_if_no_bitcoind_zmq(test_instance):
    """Skip the running test if amerod has not been compiled with zmq support."""
    config = configparser.ConfigParser()
    config.read_file(open(test_instance.options.configfile))

    if not config["components"].getboolean("ENABLE_ZMQ"):
        raise SkipTest("amerod has not been built with zmq enabled.")
